...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((results) => {
    ...
    results.forEach((element) => {
      if (element.status === 'fulfilled') arr.push(element.value);
      if (element.status === 'rejected') arr.push(null);
    });
    return arr;
  });
}
...
function queuPromises(promises) {
  ...
  promises.forEach((promise) => {
    res = res.then((result) => {
      return promise.then((newResult) => {
        return result + newResult.toString();
      });
    });
  });
  return res;
